...
#    Guarded like step (c): if no shift was found, ${LOWER:$FOUND_SHIFT} with
#    FOUND_SHIFT=-1 would silently parse as the ${VAR:-default} expansion and
#    produce a wrong encryption, so we bail out instead.
if [ "$FOUND_SHIFT" -lt 0 ]; then
    echo "Could not determine the Caesar shift; not appending an encrypted line." >&2
    exit 1
fi
