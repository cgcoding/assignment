...
- ~~Minor: if `<Word>` contained regex metacharacters, `grep` would treat them as a pattern;
  `grep -Fowi` would harden it.~~ **Fixed 2026-07-04:** the script now uses `grep -owiF`
  so the word is matched as a fixed string.
...
- ~~Step (c) is correctly guarded by `[ "$FOUND_SHIFT" -ge 0 ]`, but step (d) is **not**~~
  **Fixed 2026-07-04:** step (d) now bails out with an error (`exit 1`) when
  `FOUND_SHIFT` is `-1`, matching step (c)'s guard, so the `${LOWER:-1}` default-value
  misparse can no longer produce a silently wrong encryption.
...
- **`evaluate.sh`** - run from `q4/` (no args). Compiles each student `.cpp` from inside
  ...
  - **Note (not a defect):** `find | sort` makes `distribution.txt` line ordering
    ...
    (`LC_ALL=C sort -t, -k1,1`) so the committed artifact stays stable. **Fixed 2026-07-04:**
    both sorts in `evaluate.sh` (`find | sort` and the marksheet sort) now use `LC_ALL=C`;
    a re-run reproduced `marksheet.csv` and `distribution.txt` byte-identically.
...
- `wget` download out of scope. Built a local `encrypted.txt` (known plaintext containing
  ...
  are exact inverses.
- **Latent edge case - FIXED 2026-07-04:** step (d) was unguarded - if no shift were found
  (`FOUND_SHIFT=-1`), `${LOWER:$FOUND_SHIFT}` parses as `${LOWER:-1}` (default-value operator)
  and yields the whole alphabet. The script now exits with an error before step (d) when
  `FOUND_SHIFT < 0`, mirroring step (c)'s guard.
