...
done < <(find "$INPUTS_DIR" -maxdepth 1 -type f -name "*.in" | LC_ALL=C sort)
...
# LC_ALL=C pins the byte-wise collation so results are locale-independent.
LC_ALL=C sort "$SCRIPT_DIR/marksheet.csv" -o "$SCRIPT_DIR/marksheet.csv"
