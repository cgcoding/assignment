...
> **Update 2026-07-04 - committed tree made self-consistent.** The committed
> `q1/q1_inputs` was previously left in a *stale* mutated state from the older
> (over-matching) `replace.sh` run: `adapter.h` contained `#include <newlib.h>` even though
> it is absent from the corrected 6-file `patched_files.txt`. The tree was reset from the
> pristine copy (`q1 resources/q1_inputs`) and all five scripts were re-run **in place**:
> every file in `q1/q1_outputs/` (including `debug_removed_count.txt` = `5`) reproduced
> **byte-identically**, and `adapter.h` now correctly retains `oldlib.h` (prototype-only, not
> patched). The committed inputs and outputs are now exactly what the committed scripts
> produce.
